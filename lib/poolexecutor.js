...
export default class PoolExecutor extends EventEmitter {
    ...
    async start() {
        if (this.#tasksQueued) {
            for (let i=0; i<Math.min(this.#concurrency, this.#queue.size); i++) {
                this.#nextTask();
            }
    ...
    /**
     * Dequeue next task from queue and start execution
     */
    #nextTask() {
    ...
    /**
     * Execute a task and add its result to the result array.
