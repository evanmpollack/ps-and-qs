...
export default class PriorityQueue {
    ...
    static fromArray(array, comparator) {
        ...
        // heapify using bottom-up heap construction algorithm
        instance.#heap = (() => {
            let lastInternalIndex = Math.floor((array.length / 2)) - 1;
            for (let i=lastInternalIndex; i>=0; i--) PriorityQueue.#siftDown(array, i, instance.#comparator);
            return array;
        })();
        return instance;
    }
     ...
     */
    enqueue(data) {
        if(data === null || data === undefined) throw new TypeError('Cannot enqueue nullish data into priority queue');
        this.#heap.push(data);
